...
def find_min(array, key):
    if not array:
        return None
    return min(item[key] for item in array if item[key] is not None)


def find_max(array, key):
    if not array:
        return None
    return max(item[key] for item in array if item[key] is not None)
...
def calculate_avg(array, key):
    if array:
        sum_ = sum(item[key] for item in array)
